 ...
 * every CTA points at the App Store. Design: "Sunrise Storybook"
...
// Sona is free, so the CTA is "download it", not "buy it". This used to point
// at /api/checkout on the free build too — and GET /api/checkout 303s straight
// into a live Stripe session, so every "Get Sona — free" button on this page
// walked a parent to a payment screen. The Stripe rail stays wired for the day
// pricing returns; it just isn't what a free CTA links to.
const CHECKOUT = "https://apps.apple.com/app/id6785755867";
...
export default function Landing() {
  return (
    <main style={{ background: CREAM, color: INK, minHeight: "100vh", overflowX: "hidden" }}>
      ...
      />
      {/* Ad-funnel signal: a Lead on any app-store tap. No "paywall viewed" —
          there is no paywall to view, and firing it anyway would poison the
          one funnel number that tells us whether pricing is worth revisiting. */}
      <script
        dangerouslySetInnerHTML={{
          __html: `document.addEventListener("click",function(e){var a=e.target&&e.target.closest?e.target.closest('a[href*="apps.apple.com"]'):null;if(!a)return;try{if(window.sonaTrack)window.sonaTrack("Lead",{content_name:"web_free"});}catch(err){}},true);`,
        }}
      />
        ...
        <section id="pricing" style={{ background: "#fff", padding: "30px 20px" }}>
          <div style={{ background: "#fff", border: `3px solid ${INK}`, borderRadius: 26, boxShadow: `0 7px 0 ${INK}`, padding: "22px 20px" }}>
            ...
            <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
              ...
              <div style={{ fontSize: 15, fontWeight: 800, color: MUTED }}>no catch</div>
            </div>
      ...
      >
        <div style={{ flex: 1 }}>
          <span style={{ font: `800 16px ${B}`, color: INK }}>Free</span>

...
// Sona is free: /api/checkout refuses, so every button on the picker could
// only produce an error. A parent who lands here from a bookmark or an old
// link gets the truth and a way into the app, not a broken plan picker. The
// switch is read here, BEFORE any hook or figure in this file: the free
// notice is its own component, so while free the picker's hooks never run
// and its charter fetch never fires.
function SubscribeInner() {
  if (FREE_MODE) return <FreeNotice />;
  return <PaidPicker />;
}

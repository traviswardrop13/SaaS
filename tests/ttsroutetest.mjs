...
function expectVoice(response, provider, model, cache = "miss") {
  ...
  assert.equal(response.headers.get("x-sona-voice-revision"), "v7");
}
...
test("ElevenLabs preserves chosen voice and existing delivery, reports actual provider", async () => {
  const f = fixture();
  ...
  assert.deepEqual(f.calls[0].body.voice_settings, {
    stability: 0.55, similarity_boost: 0.85, style: 0.3, speed: 0.93, use_speaker_boost: true,
  });

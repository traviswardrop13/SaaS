...
function device(config){
  ...
  navigator.mediaDevices.getUserMedia=async()=>{const track={readyState:'live',stop(){this.readyState='ended';}};const s={getTracks:()=>[track],getAudioTracks:()=>[track]};h.streams.push(s);return s;};
  ...
  window.fetch=(url,options)=>String(url)==='/api/tts'?(h.fetches++,Promise.resolve({ok:!!config.server,status:config.server?200:503,headers:{get:name=>config.server?({'X-Sona-Voice-Provider':'elevenlabs','X-Sona-Voice-Cache':'miss','X-Sona-Voice-Model':'eleven_multilingual_v2','X-Sona-Voice-Revision':'v7'}[name]||null):null},arrayBuffer:async()=>new ArrayBuffer(96000)})):fetch(url,options);
  let sona;
...
await scenario('cold browser voice list',async()=>{
  const {context,page,errors}=await fresh();try{
    ...
    await page.evaluate(()=>__pacing.voices[1].end());await page.waitForTimeout(30);
...
await scenario('human model and interruption',async()=>{
  const {context,page,errors}=await fresh({human:true});try{
    ...
    await page.evaluate(()=>__pacing.media[1].end());await page.waitForFunction(()=>engineOn);
...
await scenario('muted practice',async()=>{
  const {context,page,errors}=await fresh({muted:true});try{
    await page.waitForFunction(()=>engineOn);

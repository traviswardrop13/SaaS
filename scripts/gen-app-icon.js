...
// save a transparent, downscaled head for re-runs / reuse
const m = Math.min(1, 1024 / Math.max(head.width, head.height)), hw = Math.round(head.width * m), hh = Math.round(head.height * m);
const headSmall = resize(head.data, head.width, head.height, hw, hh);
...
for (const [sz, name] of [[180, "apple-touch-icon.png"], [192, "icon-192.png"], [512, "icon-512.png"]]) {
  fs.writeFileSync(path.join(PUB, name), icon(headSmall, sz, 0.92));
